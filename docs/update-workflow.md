...
已启用 MIT OCW 2006 年物流系统课程目录：2026-09-22 核查 HTTP 200、robots 允许、正文 selector 和 CC BY-NC-SA 4.0 使用条件，仅记录出处、原创简述与正文指纹，不镜像讲义，定位为非商业学习。首次收录结论仅涉及目录教学覆盖，不代表当前岗位要求或原文刚刚更新。

其余四个候选来源保持停用；CIPS 程序访问 403，不绕过限制。其他来源需按实际定时用途进一步复核使用条件后再启用。
